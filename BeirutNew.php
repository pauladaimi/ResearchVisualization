...
<div class="graph" width="50%" height="50%"></div>
<div class="plot"></div>
    ...
    {imagePath: 'https://developers.google.com/maps/documentation/javascript/examples/markerclusterer/m'});
        ...
        google.maps.event.addListener(BridgeMarker, 'click', function () {
                // Marker Clicking Listener
                    ...
                    d3.selectAll(".bridge").style("visibility","visible");
                    d3.selectAll(".graph").style("visibility","visible");
                });
    ...
    $(document).ready(function()
{
    ...
    var resultHeat=[];
          ...
          $.post("HeatMap.php", { "fname": "func2", "Offset": ""+offsetHeat }, function(data){
            resultHeat=JSON.parse(data);
          });
      ...
      $.post("HeatMap.php", { "fname": "func1", "Time": clock, "Offset": ""+offsetHeat }, function(data){resultHeat=JSON.parse(data);});
        
        console.log(resultHeat);
        
        InitPlot(resultHeat);
        InitGraph(resultHeat);
        offsetHeat+=resultHeat.length;
    ...
    setInterval(function (){
        ...
        $.post("Beirut2PHP.php",{ "funcname": "func1", "Type": selectedType, "Offset": ""+offset }, function(data){
            ...
            $.post("HeatMap.php", { "fname": "func2", "Offset": ""+offsetHeat }, function(data){resultHeat=JSON.parse(data);});
              if(resultHeat.length!=0)
                {
                clock=resultHeat[2];
                //console.log(clock);
                //console.log(offsetHeat);
                ...
                UpdatePlot(resultHeat);
                UpdateGraph(resultHeat);
                offsetHeat+=resultHeat.length;
            }
        });

        updateMapPlot(result);
        updateMap(result);
    ...
    var colorsHeat=["grey","red","yellow","green","orange","pink","cyan","white","brown"];
    var sensorids=[];
    ...
    var valueline = d3.svg.line()
        ...
        .y(function(x,y) { 
            ...
            types=TypeDiv.append("Select").attr("class","Options");
    ...
    function updateMapPlot(result){

                selectedType = types.node().value;
...
var svg = d3.select("body")
    .append("svg")
        ...
        var rect = svg.append("rect").attr("width", "100%").attr("height", "100%").attr("fill", "black").attr('class', 'bridge');

        var g= svg.append("g").attr("class","bridge")
        .attr("transform", 
        ...
        /* Define the line
            ...
            d3.select(".bridge").style("visibility","hidden");
...
function UpdatePlot( data )
{
  for(var datacntr=0; datacntr<data.length; datacntr++)
  {
    ...
    d3.select(".xh.axis.heat").transition() // change the x axis
    .call(xAxisHeat);

    d3.select(".yh.axis.heat").transition() // change the y axis
    .call(yAxisHeat);
  //}
}
}
...
function InitGraph(data)
{
  ...
  var graphCanvas=d3.select(".graph").append("svg").attr("class","graph").attr("width", width).attr("height", height).attr("transform", "translate(" + margin.left + "," + margin.top + ")");

  canvas = graphCanvas.append("g");
  ...
  for(var i=0; i<data.length; i++){
    canvas.append("circle").attr("cx",data[i][4]+'%').attr("cy",data[i][3]+'%').attr("r",15).attr("value",i+1).attr("checked","true").style("fill",colorsHeat[i]).style("opacity","0.7").on("click",function(){
      ...
      if(selected=="true"){
        ...
        d3.select(this).style("fill",colorsHeat[selectedSensor-1]);
  ...
  var svgContainer = d3.select(".graph").append("svg").attr("width", rectWidth * legend.length + x_axis).attr("height", 500).attr("transform", "translate(" + 0 + "," + 300 + ")");
